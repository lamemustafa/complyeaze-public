...
const repo =
  explicitRepo ??
  runText(["repo", "view", "--json", "nameWithOwner", "-q", ".nameWithOwner"]);
const prNumber = Number(
  explicitPr ?? runText(["pr", "view", "--json", "number", "-q", ".number"]),
);

if (!repo || !repo.includes("/"))
  fail("Could not determine GitHub repo. Pass --repo owner/name.");
if (!Number.isInteger(prNumber) || prNumber < 1)
  fail("Could not determine PR number. Pass --pr <number>.");

const { pr, unresolvedThreads, blockingReviews, headReviews } =
...
if (missingHeadReview) {
  ...
  if (allowMissingHeadReview) {
    ...
    console.warn(
      `${message} Continuing because --allow-missing-head-review was set.`,
    );
  } else {
...
function evaluatePullRequestReviewState(pr) {
  ...
  const authorStates = reduceSubmittedCurrentHeadReviewsByAuthor(
    pr.reviews.nodes,
    pr.headRefOid,
  );
  ...
  const headReviews = Array.from(authorStates.values())
    ...
    .filter(
      (review) =>
        ...
        normaliseAuthorLogin(review.author?.login) ===
          normaliseAuthorLogin(requiredReviewAuthor),
    );
...
function reduceSubmittedCurrentHeadReviewsByAuthor(reviews, headRefOid) {
  ...
  const submittedReviews = reviews
    .filter(
      (review) => review.state !== "PENDING" && review.state !== "DISMISSED",
    )
  ...
  for (const review of submittedReviews) {
    ...
    const latestCurrentHeadReview = isCurrentHeadReview
      ? review
      : previous.latestCurrentHeadReview;
...
function reportBlockingState({ unresolvedThreads, blockingReviews }) {
  if (unresolvedThreads.length > 0) {
    ...
    for (const thread of unresolvedThreads) {
      ...
      console.error(
        `- ${thread.path}:${thread.line ?? "?"} ${comment?.url ?? thread.id}`,
      );
  ...
  if (blockingReviews.length > 0) {
    ...
    for (const review of blockingReviews) {
      console.error(
        `- ${review.author?.login ?? "unknown"} ${review.submittedAt} ${review.url}`,
      );
    }
  }
}
...
function fetchReviewGraph() {
  ...
  if (fixture)
    return normaliseFixtureReviewGraph(
